...
export function HabitActionModal({
  ...
  const rowCls = 'flex items-center gap-3 px-5 py-3.5 w-full text-left hover:bg-[var(--surface-2)] active:bg-[var(--surface-3)] transition-colors cursor-pointer select-none'
  const btnCls = 'flex-1 px-4 py-2.5 text-sm font-medium rounded-[10px] transition-colors'
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      <div
        ...
        role="dialog"
        aria-modal="true"
        className="relative w-full sm:w-80 bg-[var(--surface)] rounded-t-[20px] sm:rounded-[16px] shadow-[0_-4px_40px_rgba(0,0,0,0.12)] sm:shadow-[0_20px_60px_rgba(0,0,0,0.15)] border border-[var(--sep)] overflow-hidden animate-modal-in"
      >
        ...
        {view === 'menu' && (
          <>
            <div className="flex items-center gap-3 px-5 py-4">
              ...
              <button
                onClick={onClose}
                className="p-1.5 rounded-[8px] hover:bg-[var(--surface-2)] transition-colors shrink-0"
              >
            ...
            <button className={rowCls} onClick={onEdit}>
              <Pencil className="w-4 h-4 text-[var(--label-3)] shrink-0" />
            ...
            {isToday && !isSkipped && (
              isPostponed ? (
                <button className={rowCls} onClick={onUndoException}>
                  <RotateCcw className="w-4 h-4 text-[var(--warning)] shrink-0" />
                ...
                <button className={rowCls} onClick={() => setView('postpone')}>
                  <CalendarClock className="w-4 h-4 text-[var(--label-3)] shrink-0" />
                  <span className="flex-1 text-sm font-medium text-[var(--label)]">{t('habits.postpone')}</span>
                  <ChevronRight className="w-4 h-4 text-[var(--label-3)] shrink-0" />
                </button>
              )
            )}
            ...
            {!isFutureDate && !isPostponed && (
              isSkipped ? (
                <>
                  ...
                  <button className={rowCls} onClick={onUndoException}>
                    <RotateCcw className="w-4 h-4 text-[var(--label-3)] shrink-0" />
                ...
                <button className={rowCls} onClick={() => setView('skip')}>
                  <Ban className="w-4 h-4 text-[var(--label-3)] shrink-0" />
            ...
            {habit.habitType === 'measurable' && habit.step != null && currentValue > 0 && !isFutureDate && !isSkipped && !isPostponed && (
              <button className={rowCls} onClick={onDecrement}>
                <Minus className="w-4 h-4 text-[var(--label-3)] shrink-0" />
            ...
            <button className={rowCls} onClick={() => setView('delete')}>
              ...
              <ChevronRight className="w-4 h-4 text-[var(--danger)] shrink-0" />
            </button>
          ...
          <div className="p-5">
            ...
            <form onSubmit={(e) => { e.preventDefault(); onSkip(habit.id, skipNote) }}>
              ...
              <input
                ...
                className="w-full px-3 py-2.5 text-sm rounded-[10px] bg-[var(--surface-2)] border border-[var(--sep)] text-[var(--label)] placeholder:text-[var(--label-3)] focus:outline-none focus:ring-2 focus:ring-[var(--accent)] mb-3"
              />
